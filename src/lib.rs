...
//! assert_eq!(context.get_value("a").map(std::borrow::Cow::into_owned), Some(Value::from(5)));
...
//! assert_eq!(context.get_value("a").map(std::borrow::Cow::into_owned), Some(5.into()));
//! ```
//!
...
//! assert_eq!(context.get_value("a").map(std::borrow::Cow::into_owned), Some(Value::from(10)));
//! assert_eq!(context.get_value("b").map(std::borrow::Cow::into_owned), Some(Value::from(1.0)));
//! ```
//!
...
pub use crate::{
    context::{
        ...
        TransposeColumnIndex,
        BoxedTransposeColumnIndex,
        BoxedTransposeColumnIndexHolder,
        BoxedOperatorRowTrait,
        OperatorStatusContainerTrait,
    ...
    token::PartialToken,
    tree::Node,
    value::{value_type::ValueType, EmptyType, FloatType, IntType, TupleType, Value, EMPTY_VALUE, FfiResult, to_ffi_result,to_ffi_result_func, to_nested_ffi_result,Error},
};
...
pub use crate::{
    context::{
        ...
        OperatorRowTrait_get_value_for_column,
        OperatorRowTrait_set_value_for_column,
        OperatorRowTrait_set_row,
        OperatorRowTrait_call_function,
        OperatorRowTrait_has_changes,
        OperatorRowTrait_get_dirty_flags,
        OperatorSchemaTrait_get_schema,
        OperatorSchemaTrait_get_column_for_index,
        OperatorSchemaTrait_get_index_for_column,
        OperatorSchemaTrait_add_column,
        OperatorSchemaTrait_get_value,
        OperatorSchemaTrait_set_value,
        OperatorSchemaTrait_get_value_for_column,
        OperatorSchemaTrait_set_value_for_column,
