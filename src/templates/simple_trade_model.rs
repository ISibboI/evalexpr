                ...
                )
            {
                ...
                if loop_active_trade {
                    ...
                    if loop_stop_loss.is_some_and(|sl| current_close_value <= sl) {
                        ...
                        reason = Some(format!("{} {} Closing trade. Current price ({}) has fallen to or below stop loss {} from entry price ({}).", get_delta_label(delta_value), delta_value, current_close_value, loop_stop_loss.as_ref().unwrap(), loop_initiation_price));
                    } else if  loop_take_profit.is_some_and(|tp| current_close_value >= tp) {
                        loop_trade_closed = true;
                        exit_price = Some(current_close_value);
                        let delta_value = (current_close_value - loop_initiation_price) * loop_conviction;
                        delta = Some(delta_value);
                        reason = Some(format!("{} {} Closing trade. Current price ({}) has reached or exceeded take profit level {} from entry price ({}).",get_delta_label(delta_value),delta.unwrap(), current_close_value,loop_take_profit.as_ref().unwrap(), loop_initiation_price));
