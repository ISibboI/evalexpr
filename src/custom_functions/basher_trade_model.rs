...
impl CompiledTransposeCalculationTemplate for BasherTradeModel {
    ...
    fn commit_row(&self, row: &mut BoxedThinTraitContext, ordered_transpose_values: &[Value], cycle_epoch: usize) -> Result<(), Error> {
        ...
        for i in cycle_epoch ..ordered_transpose_values.len() {
            ...
            let current_close_value = row.get_value(&generate_column_name("close", transpose_value))?.as_float()?;
            ...
            if loop_active_trade {
                ...
                if current_close_value <= loop_stop_loss {
                    ...
                    stop_loss = Some(loop_initiation_price + 0.0001);
                }
                ...
                if loop_trade_signal {
                    if !prev_trade_signal.unwrap_or_default() {
                        ...
                        stop_loss = Some(current_close_value - 0.0002);
                        take_profit = Some(current_close_value + 0.0003);
                        break_even = Some(current_close_value + 0.0001);
